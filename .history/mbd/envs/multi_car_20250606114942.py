...
def compute_r_safe(p, q_all, k, Ra, margin=0.05):
    ...
    safe_d = 2 * Ra + margin
    decay = 5.0  # puoi aumentarlo per renderlo più ripido
...
class MultiCar2d:
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        # Blocco per collisioni tra robot
        
        # def clip_if_robot_collides(q_next, q_curr, all_q_next, Ra):
        #     # Distanze da tutti gli altri robot
        #     dists = jnp.linalg.norm(all_q_next[:, :2] - q_next[:2], axis=1)
        #     # Escludi se stessi usando una soglia minima > 0
        #     self_mask = dists < 1e-6
        #     # Collisione se distanza < 2 * Ra e non è se stesso
        #     collided = jnp.any(jnp.logical_and(dists < 2 * Ra, ~self_mask))
        #     return jax.lax.select(collided, q_curr, q_next)
    
        # q_new = jax.vmap(clip_if_robot_collides, in_axes=(0, 0, None, None))(q_new, q, q_new, self.Ra)
