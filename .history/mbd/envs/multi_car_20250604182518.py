...
def log_barrier_pairwise_centered(p_i, all_pos, k, Ra, eps=1e-3):
    # all_pos: (n, 2), p_i: (2,), k: int
    # distanza tra p_i e tutti
    dists = jnp.linalg.norm(p_i - all_pos, axis=1)
    # crea maschera con tutti tranne il robot k
    mask = jnp.arange(all_pos.shape[0]) != k
    dists_no_self = jnp.where(mask, dists, 1e6)  # ignora il robot k
    dist_safe = jnp.clip(dists_no_self - 2 * Ra, a_min=eps)
    return jnp.mean(-jnp.log(dist_safe))
