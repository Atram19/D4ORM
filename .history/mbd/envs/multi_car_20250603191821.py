...
class MultiCar2d:
    def __init__(self, n, radius=2.0, robot_radius=0.1,formation_shift = False,obstacles_enabled=False):
        ...
        if obstacles_enabled:
            self.static_obstacles = jnp.array([
                [0.0,  0.0, 1.5, 0.2],  # barra orizzontale centrale
                [0.0,  0.5, 1.5, 0.2],  # barra orizzontale sopra
                [0.0, -0.5, 1.5, 0.2],  # barra orizzontale sotto
                [ 0.0,  0.0, 0.2, 1.5], # barra verticale centrale
            ])
        else:
            self.static_obstacles = jnp.zeros((0, 4))
