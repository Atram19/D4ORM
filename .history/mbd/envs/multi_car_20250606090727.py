...
def log_barrier_pairwise(p, q_all, k, Ra, margin=0.05, epsilon=1e-3):
    others = q_all[:, :2]
    dists = jnp.linalg.norm(p - others, axis=1)  # (n,)
    safe_d = 2 * Ra + margin

    # Maschera per escludere il robot k-esimo
    is_other = jnp.arange(q_all.shape[0]) != k
    penalties = -jnp.log(jnp.clip((safe_d - dists) / margin, a_min=epsilon))
    penalties = jnp.where(dists < safe_d, penalties, 0.0)
    masked = jnp.where(is_other, penalties, 0.0)
...
class MultiCar2d:
    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            def single_obs_penalty_logbar(p, obstacle, margin=0.05, epsilon=1e-3):
                xc, yc, w, h = obstacle
                x_min = xc - w / 2
                x_max = xc + w / 2
                y_min = yc - h / 2
