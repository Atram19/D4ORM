...
class MultiCar2d:
    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            def single_obs_penalty(p, obstacle, margin=0.05, Ra=0.1):
                """
                Penalizza solo quando il robot è vicino all'ostacolo (entro soglia),
                con una log-barrier normalizzata che va da -1 a 0.
                """
                ...
                penalty = jnp.where(
                    dist < safe_d,
                    -jnp.log(jnp.clip((safe_d - dist) / margin, a_min=1e-6)),  # tra -1 e 0
                    0.0
                )
            ...
            if self.obstacles_enabled:
                obstacles = jnp.array(self.static_obstacles)
                r_obs_vals = jax.vmap(lambda obs: single_obs_penalty(p, obs, margin=0.05, Ra=self.Ra))(obstacles)
