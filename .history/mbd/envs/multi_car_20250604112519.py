...
class MultiCar2d:
    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            np.savez("results/multicar_iterative/global_rewards_debug.npz",
            r_goal=np.array(r_goal_seq),
            r_safe=np.array(r_safe_seq),
            r_form=np.array(r_form_seq),
            r_control=np.array(r_control_seq),
            r_obstacles=np.array(r_obs_seq),
            total_reward=np.array(r_total_seq))
