    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            def single_obs_penalty(p, obstacle):
                ...
                # Penalità extra se in collisione netta (p dentro ostacolo)
                def is_inside(p, obstacle):
                    x_c, y_c, w, h = obstacle
                    return (p[0] >= x_c - w/2 - self.Ra) & (p[0] <= x_c + w/2 + self.Ra) & \
                        (p[1] >= y_c - h/2 - self.Ra) & (p[1] <= y_c + h/2 + self.Ra)

                collisions = jax.vmap(lambda obs: is_inside(p, obs))(obstacles)
                hard_penalty = -5.0 * jnp.any(collisions)
                r_obstacles += hard_penalty
                return -1.0 / (1.0 + jnp.exp(40 * (dist - (self.Ra + 0.15))))
