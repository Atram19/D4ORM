    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            def single_obs_reward(p, obstacle, pT):
                """
                Penalizza l’ingresso in un ostacolo con log-barrier soft,
                premia poco l’aggiramento utile verso il goal.
                """
                ...
                # Penalità log-barrier se dentro (più liscia, graduale)
                barrier_penalty = -10.0 * jnp.log(dist_to_obstacle + 1e-3)

                # shaping: premio lieve se vicino, non diverge
                reward_if_outside = 1.0 * jnp.exp(-4.0 * dist_to_obstacle) * jnp.exp(-0.3 * dist_to_goal)

                reward = jnp.where(is_inside, barrier_penalty, reward_if_outside)
                return reward
