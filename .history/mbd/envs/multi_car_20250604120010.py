...
class MultiCar2d:
    ...
    def get_reward_trajectory_terms(self, q_seq, u_seq):
        """
        Calcola reward frame per frame lungo una traiettoria.
        q_seq: (H, n, 3)  -> stati
        u_seq: (H, n, 2)  -> controlli
        Output: dict con shape (H,) per ogni termine della reward
        """
        def reward_at_t(q, u, q_all):
            return self.get_reward_terms(q_all, u)  # Usa già il vmap su robot

        # Applica a ogni tempo
        rewards = jax.vmap(lambda qt, ut: reward_at_t(qt, ut, qt))(q_seq, u_seq)
        return rewards  # shape (H, n, 6)
