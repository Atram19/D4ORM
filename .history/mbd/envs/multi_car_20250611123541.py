...
def check_collision_static(pos, obstacles, Ra):
    def single_obs_check(obs):
        ...
        # Verifica se il centro del robot entra nel rettangolo "espanso"
        return (x_min <= pos[0]) & (pos[0] <= x_max) & (y_min <= pos[1]) & (pos[1] <= y_max)
...
class MultiCar2d:
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        if self.obstacles_enabled:
            def clip_static(q_next_i, q_curr_i):
                collided = check_collision_static(q_next_i[:2], self.static_obstacles, self.Ra)
                return jax.lax.select(collided, q_curr_i, q_next_i)
