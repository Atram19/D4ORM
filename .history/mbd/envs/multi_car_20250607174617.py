    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            def single_obs_reward(p, obstacle):
                ...
                # Distanza dall’esterno (se fuori) o 0 (se dentro)
                dx = jnp.maximum(jnp.maximum(obs_x_min - p[0], 0.0), p[0] - obs_x_max)
                dy = jnp.maximum(jnp.maximum(obs_y_min - p[1], 0.0), p[1] - obs_y_max)
                dist = jnp.sqrt(dx**2 + dy**2 + 1e-6)

                # Controlla se il robot è dentro l’ostacolo (tocco)
                is_inside_x = jnp.logical_and(p[0] >= obs_x_min, p[0] <= obs_x_max)
                is_inside_y = jnp.logical_and(p[1] >= obs_y_min, p[1] <= obs_y_max)
                is_inside = jnp.logical_and(is_inside_x, is_inside_y)

                # Penalità se dentro (es. -20), premio se fuori proporzionale alla vicinanza (es. max ~+5)
                penalty_if_inside = -20.0
                reward_if_outside = 5.0 / (dist + 0.1)  # cresce vicino all’ostacolo ma non diverge
            ...
            if self.obstacles_enabled:
                ...
                r_obs_vals = jax.vmap(lambda obs: single_obs_rewards(p, obs))(obstacles)
                r_obstacles = jnp.mean(jnp.max(r_obs_vals, axis=-1))  # max per robot, poi media
            ...
            r_move = jnp.linalg.norm(u_k)  # premia il movimento


            return 10 * r_goal + 40 * r_safe +20 * r_obstacles + 0.25 * r_form + 0 * r_control+ 0.1 * r_move
