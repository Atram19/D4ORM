...
def check_collision_static(pos, obstacles, Ra):
    """
    Verifica se il punto `pos` collide con uno degli ostacoli rettangolari statici.
    pos: (2,)
    obstacles: (n_obs, 4) con (xc, yc, w, h)
    Ra: raggio di sicurezza del robot
    """
...
class MultiCar2d:
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        if self.obstacles_enabled:
            obstacles = jnp.array(self.static_obstacles)  # (n_obs, 4)
    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            def single_obs_penalty(p, obstacle):
                ...
                return -jnp.exp(-200.0 * (dist - (self.Ra + 0.1)))
            if self.obstacles_enabled:
