...
class MultiCar2d:
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        def clip_if_robot_collides(q_next, q_curr, all_q_next):
            collided = jnp.any(jnp.linalg.norm(all_q_next[:, :2] - q_next[:2], axis=1) < 2 * self.Ra)
            self_idx = jnp.all(q_next == all_q_next, axis=1)
            collided = jnp.logical_and(collided, ~self_idx)
            return jax.lax.select(collided, q_curr, q_next)
        q_new = jax.vmap(clip_if_robot_collides, in_axes=(0, 0, None))(q_new, q, q_new)
