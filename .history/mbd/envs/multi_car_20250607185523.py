    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            def single_obs_reward(p, obstacle, pT):
                ...
                # Penalità e reward
                penalty_if_inside = -50*(1/ (1.0 + jnp.exp(100 * (dist_to_obstacle - 2*self.Ra))))

               # Piccolo shaping positivo se in zona utile per l’aggiramento
                shaping = jnp.exp(-4.0 * (dist_to_obstacle-0.2)) * jnp.exp(-0.3 * dist_to_goal)  # max ~1
                reward_if_outside = 2.0 * shaping  # massimo circa 2

                reward = jnp.where(is_inside, penalty_if_inside, reward_if_outside) # massimo circa 2
                return reward
            def is_near_obstacle(p, obs, threshold=0.5):
                x_c, y_c, w, h = obs
                center = jnp.array([x_c, y_c])
                return jnp.linalg.norm(p - center) < threshold


            if self.obstacles_enabled:
                obstacles = jnp.array(self.static_obstacles)
                threshold = 0.8  # metri
                near_mask = jax.vmap(lambda obs: is_near_obstacle(p, obs, threshold))(obstacles)

                # Applica reward solo sugli ostacoli vicini
                def masked_obs_reward(obs, mask):
                    return jnp.where(mask, single_obs_reward(p, obs, pT), 0.0)

                r_obs_vals = jax.vmap(masked_obs_reward)(obstacles, near_mask)
                r_obstacles = jnp.max(r_obs_vals)  # o mean, se preferisci
            ...
            return 10 * r_goal + 40 * r_safe + r_obstacles + 0.25 * r_form + 0.1 * r_control
