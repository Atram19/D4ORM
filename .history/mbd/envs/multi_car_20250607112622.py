...
class Args:
    seed: int = 0
    n_robots: int = 4
    Nsample: int = 3096         # number of samples
    ...
    betaT: float = 0.02         # final noise  # PER ECD DEVE ESSERE A 1 SE NO NON VAA
    # PER D4orm e ECD
    # beta0: float = 1e-4         # initial noise
...
class MultiCar2d:
    ...
    def get_rewards(self, q_all, u_all):
        def single_reward(k, q, u_all):
            ...
            # 1. Goal tracking
            r_goal = 1.0 - jnp.linalg.norm(p - pT) / (jnp.linalg.norm(p0 - pT) + 1e-6)
            ...
            if self.ECD or self.obstacles_enabled:
                ...
                mask = (jnp.arange(self.n) != k)
                r_safe = -1.0 * jnp.any((dists <= 2 * self.Ra + 1e-2) & mask)

            # 3. Static obstacle penalty
            if self.obstacles_enabled:
                ...
                def penalty(p, obs):
                    ...
                    dx = jnp.maximum(jnp.maximum(x_min - p[0], 0.0), p[0] - x_max)
                    dy = jnp.maximum(jnp.maximum(y_min - p[1], 0.0), p[1] - y_max)
                    dist = jnp.sqrt(dx**2 + dy**2 + 1e-6)
                    return -jnp.exp(-20.0 * (dist - (self.Ra + 0.1)))
                r_obs_vals = jax.vmap(lambda obs: penalty(p, obs))(obstacles)
                r_obstacles = jnp.mean(r_obs_vals)
            else:
                r_obstacles = 0.0

            # 4. Formation (optional)
            if self.formation_shift:
                pos = q_all[:, :2]
                pos0 = self.x0[:, :2]
                ...
                r_form = -jnp.mean((dists - dists0) ** 2 * mask)
            else:
                r_form = 0.0
