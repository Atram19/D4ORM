...
def check_collision_full(X_t, obstacles, Ra):
    """
    Verifica se c'è collisione con ostacoli statici o tra robot.
    """
    n = X_t.shape[0]

    # Collisioni con ostacoli statici
    def check_single_robot_static(pos):
    ...
    any_static_collision = jnp.any(jax.vmap(check_single_robot_static)(X_t[:, :2]))

    # Collisioni tra robot
    pos = X_t[:, :2]
    dists = jnp.linalg.norm(pos[:, None, :] - pos[None, :, :], axis=-1)
    collision_matrix = dists < 2 * Ra
    collision_matrix = collision_matrix.at[jnp.diag_indices(n)].set(False)
    any_inter_robot_collision = jnp.any(collision_matrix)

    return any_static_collision | any_inter_robot_collision
...
class MultiCar2d:
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        # Previsione prossimo stato
        q_new = jax.vmap(rk4, in_axes=(None, 0, 0, None))(
            car_dynamics, q, action, self.dt
        )

        # Verifica collisioni globali
        collision = check_collision_full(q_new, self.static_obstacles, self.Ra)

        # Blocca evoluzione se collisione (tutti i robot fermi)
        q_next = jnp.where(collision, q, q_new)

        reward = self.get_rewards(q_next, action)
        return state.replace(pipeline_state=q_next, obs=q_next, reward=reward, done=jnp.zeros((self.n,)))
    ...
    def get_reward_terms(self, q_all, u_all):
        def single_reward_terms(k, q, u_all):
            ...
            def single_obs_penalty(p, obstacle):
                ...
                dy = jnp.maximum(jnp.maximum(obs_y_min - p[1], 0.0), p[1] - obs_y_max)
                dist = jnp.sqrt(dx**2 + dy**2 + 1e-3)
                return -jnp.exp(-15.0 * (dist - (self.Ra + 0.1)))
