    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            def single_obs_penalty(p, obstacle):
                ...
                # soft_penalty = -1.0 / (1.0 + jnp.exp(100 * (dist - (2*self.Ra))))

                # penalità log-barrier soft (sempre negativa, maggiore vicino all’ostacolo)
                alpha = 5.0  # puoi aumentare se vuoi più forza
                eps = 1e-2
                penalty = -alpha / (dist + eps)
                return penalty
            ...
            return 10 * r_goal + 40 * r_safe +40 * r_obstacles + 0.25 * r_form + 0 * r_control+ 0.1 * r_move
