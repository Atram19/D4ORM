    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            def single_obs_reward(p, obstacle, pT):
                ...
                # Penalità e reward
                penalty_if_inside = -50-1.0 / (1.0 + jnp.exp(100 * (dist_to_obstacle - 2*self.Ra)))

                reward_if_outside = 5.0 / (dist_to_obstacle + 0.1) * jnp.exp(-0.3 * dist_to_goal)

                reward = jnp.where(is_inside, penalty_if_inside, reward_if_outside)
                return reward
