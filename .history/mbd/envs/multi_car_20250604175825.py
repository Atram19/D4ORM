...
class MultiCar2d:
    ...
    def log_barrier_pairwise(p_k, others_pos, Ra, epsilon=1e-6):
        dists = jnp.linalg.norm(p_k - others_pos, axis=-1)
        safe_dists = jnp.clip(dists - 2 * Ra, a_min=epsilon)
        barrier = -jnp.log(safe_dists)
    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            others_pos = jnp.delete(q[:, :2], k, axis=0)
            r_safe = -log_barrier_pairwise(p, others_pos, self.Ra)
    ...
    def get_reward_terms(self, q_all, u_all):
        def single_reward_terms(k, q, u_all):
            ...
            # dists = jnp.linalg.norm(p - q_all[:, :2], axis=1)
            # r_safe = -1.0 * jnp.any((dists <= 2 * self.Ra + 1e-2) & (jnp.arange(self.n) != k))
            # r_safe in forma continua tipo log-barrier
            others_pos = jnp.delete(q_all[:, :2], k, axis=0)
            r_safe = -log_barrier_pairwise(p, others_pos, self.Ra)
            def rews_formation(q_all, x0_all):
                pos = q_all[:, :2]
                pos0 = x0_all[:, :2]
            ...
            r_total = r_goal + self.wt * r_safe + r_form + 0.1 * r_control + r_obstacles

            return jnp.array([r_goal, r_safe, r_form, r_control, r_obstacles, r_total])

        return  jax.vmap(single_reward_terms, in_axes=(0, 0, 0, None))(jnp.arange(self.n), q_all, u_all, q_all)
