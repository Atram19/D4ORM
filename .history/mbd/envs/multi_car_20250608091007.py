...
class MultiCar2d:
    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            if self.ECD or self.obstacles_enabled:
                ...
                is_other = jnp.arange(q_all.shape[0]) != k  # per ignorare self-distance
                penalties = -1.0 / (1.0 + jnp.exp(100 * (dists - 2*self.Ra)))
                r_safe = jnp.sum(penalties * is_other)
            ...
            # Reward shaping per ostacoli statici: penalizza l'ingresso, premia l'aggiramento utile verso il goal
            def single_obs_reward(p, obstacle, pT):
                """
                Reward per ostacolo statico:
                - penalità se dentro l’ostacolo
                - reward positiva se vicino e il goal è vicino

                Args:
                    p: posizione robot (x, y)
                    obstacle: (x_c, y_c, w, h)
                    p_goal: posizione goal (x, y)

                Returns:
                    reward: negativa se dentro, positiva se vicino in modo utile
                """
                ...
                # Distanza Euclidea da bordo ostacolo
                dx = jnp.maximum(jnp.maximum(obs_x_min - p[0], 0.0), p[0] - obs_x_max)
                dy = jnp.maximum(jnp.maximum(obs_y_min - p[1], 0.0), p[1] - obs_y_max)
                dist_to_obstacle = jnp.sqrt(dx**2 + dy**2 + 1e-6)

                # Controllo se dentro l’ostacolo
                inside_x = jnp.logical_and(p[0] >= obs_x_min, p[0] <= obs_x_max)
                inside_y = jnp.logical_and(p[1] >= obs_y_min, p[1] <= obs_y_max)
                is_inside = jnp.logical_and(inside_x, inside_y)

                # Distanza dal goal (per modulare la reward)
                dist_to_goal = jnp.linalg.norm(p - pT)

                # Penalità e reward
                penalty_if_inside = -500

               # Piccolo shaping positivo se in zona utile per l’aggiramento
                # shaping = jnp.exp(-4.0 * (dist_to_obstacle-0.2)) * jnp.exp(-0.3 * dist_to_goal)
                # reward_if_outside = 2.0 * shaping  # massimo circa 2

                reward =penalty_if_inside
                return reward
            ...
            if self.obstacles_enabled:
                ...
                def reward_from_obs(obs):
                    return single_obs_reward(p, obs, pT)

                r_obs_vals = jax.vmap(reward_from_obs)(obstacles)
                r_obstacles = jnp.max(r_obs_vals)  # massimo reward da ostacoli statici
            ...
            return 10 * r_goal + 20 * r_safe + r_obstacles + 0.25 * r_form + 0.1 * r_control
    ...
    def get_reward_terms(self, q_all, u_all):
        def single_reward_terms(k, q, u_all):
            ...
            r_total=  10 * r_goal + 0.5 * r_safe + 0.5 * r_obstacles + 0.25 * r_form + 0.1 * r_control
