...
def car_dynamics(x, u):
    ...
    return jnp.array(
        [
            u[1] * jnp.cos(x[2])*1.0,  # x_dot  # CAMBIO DINAMICA
            u[1] * jnp.sin(x[2])*1.0,  # y_dot
...
def is_safe_transition(q_start, q_end, static_obstacles, Ra, n_substeps=5):
    # Interpolazione lineare tra q_start e q_end (sottopassi)
    qs = jnp.linspace(q_start, q_end, n_substeps)
    # Verifica se uno qualsiasi dei sottopassi collide
    return jnp.all(
        jax.vmap(lambda q: ~check_collision_static(q[:2], static_obstacles, Ra))(qs)
    )
...
class MultiCar2d:
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        #         safe = is_safe_transition(q_curr, q_next, self.static_obstacles, self.Ra, n_substeps=10)
        #         return jax.lax.select(safe, q_next, q_curr)
        # if self.obstacles_enabled:
        #     def clip_static(q_next, q_curr):
        ...
        # DEBUG: stampa i due
        reward_vec, r_terms = self.get_rewards(q_new, action)
        print(f"→ step reward:", reward_vec)
        
        jax.debug.print("→ r_total: {}", reward_vec)
        print(f"→ r_total from r_terms:", r_terms[:, -1])
    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_k):
            ...
            def single_obs_penalty(p, obstacle):
                ...
                dist = jnp.sqrt(dx**2 + dy**2 + 1e-6)
                #return -jnp.exp(-20.0 * (dist - (2*self.Ra )))
               
                x_clipped = jnp.minimum(dist, 1.0)  # CORRETTO!
                barrier = jnp.log(x_clipped) / jnp.log(2*self.Ra)
                #safe_dist = jnp.clip(10*(dist - 2*self.Ra) , a_min=1e-4)
                penalty = -100*barrier
                return penalty
            if self.obstacles_enabled:
            ...
            r_total = r_goal + self.wt * r_safe + r_form + 0.1*r_control+1* r_obstacles
            return r_total, jnp.array([r_goal, r_safe, r_form, r_control, r_obstacles, r_total])
