...
class Args:
    ...
    # PER D4orm e ECD e vis
    beta0: float = 1e-4         # initial noise
    betaT: float = 1e-2         # final noise  

    initial_sigma: float = 1   # initial gaussian noise # ECD = 1
    alpha: float = 0.3        # optimization step size
    mu: float = 50             # penalty term
    noise_decay: float = 0.01    # decay factor for noise # ECD = 0.3
...
class MultiCar2d:
    ...
    #             # Penalità e reward
    #             alpha = 2.0  # puoi aumentare se vuoi più forza
    #             eps = 1e-2
    #             penalty = -alpha / (dist_to_obstacle + eps)
    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            return r_goal + self.wt * r_safe + r_form + 0.1*r_control+1* r_obstacles
