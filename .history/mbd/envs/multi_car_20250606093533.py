...
def log_barrier_pairwise(p, q_all, k, Ra, margin=0.05, epsilon=1e-6):
    others = q_all[:, :2]
    dists = jnp.linalg.norm(p - others, axis=1)
    safe_d = 2 * Ra + margin
    is_other = jnp.arange(q_all.shape[0]) != k
    penalties = -jnp.log(jnp.clip((safe_d - dists) / margin, a_min=epsilon))
    masked = jnp.where((dists < safe_d) & is_other, penalties, 0.0)
    return jnp.sum(masked)  # somma forte, non media
...
class MultiCar2d:
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        #     def clip_if_collides(q_next, q_curr):
        #         collided = check_collision_static(q_next[:2], obstacles, self.Ra)
        #         return jax.lax.select(collided, q_curr, q_next)

        #     q_new = jax.vmap(clip_if_collides)(q_new, q)
    ...
    def get_rewards(self, q_all, u_all):
        def single_reward(k, q, u_all, q_all, x0_all, xg_all, obstacles, Ra, formation_shift, wt=2.0):
            u_k = u_all[k]
            p = q[:2]
            pT = xg_all[k][:2]
            p0 = x0_all[k][:2]
            ...
            # ===== Penalità inter-robot log-barrier =====
            def log_barrier_pairwise(p, q_all, k, Ra, margin=0.05, epsilon=1e-6):
                others = q_all[:, :2]
                dists = jnp.linalg.norm(p - others, axis=1)
                safe_d = 2 * Ra + margin
                ...
                penalties = -jnp.log(jnp.clip((safe_d - dists) / margin, a_min=epsilon))
                masked = jnp.where((dists < safe_d) & is_other, penalties, 0.0)
                return jnp.sum(masked)  # somma → penalità più forte

            r_safe = log_barrier_pairwise(p, q_all, k, Ra)

            # ===== Penalità ostacoli statici log-barrier =====
            def single_obs_penalty_logbar(p, obs, margin=0.05, epsilon=1e-6):
                xc, yc, w, h = obs
                x_min, x_max = xc - w / 2, xc + w / 2
                y_min, y_max = yc - h / 2, yc + h / 2
                dx = jnp.maximum(jnp.maximum(x_min - p[0], 0), p[0] - x_max)
                dy = jnp.maximum(jnp.maximum(y_min - p[1], 0), p[1] - y_max)
                dist = jnp.sqrt(dx ** 2 + dy ** 2 + epsilon)
                safe_d = Ra + margin
                return -jnp.log(jnp.clip((safe_d - dist) / margin, a_min=epsilon)) if dist < safe_d else 0.0

            r_obstacles = jnp.sum(jax.vmap(lambda obs: single_obs_penalty_logbar(p, obs))(obstacles))

            # ===== Penalità sulla formazione (se attiva) =====
            def rews_formation(q_all, x0_all):
                ...
                mask = jnp.triu(jnp.ones((q_all.shape[0], q_all.shape[0]), dtype=bool), k=1)
                return jnp.sum((dists - dists0) ** 2 * mask)

            r_form = -rews_formation(q_all, x0_all) if formation_shift else 0.0

            # ===== Penalità “soft-hard” se collisione grave =====
            max_violation = jnp.maximum(r_safe, r_obstacles)
            penalty_scale = 1.0 / (1.0 + jnp.exp(-10.0 * (max_violation - 5.0)))  # attiva sopra soglia
            r_hard_penalty = -500.0 * penalty_scale  # penalità differenziabile

            # ===== Reward finale =====
            r_total = r_goal + 0.1 * r_control + wt * r_safe + 50.0 * r_obstacles + r_form + r_hard_penalty

            return r_total
        return jax.vmap(
            lambda k, q: single_reward(
                k, q, u_all, q_all, self.x0, self.xg, self.static_obstacles, self.Ra, self.formation_shift, self.wt
            ),
            in_axes=(0, 0)
        )(jnp.arange(self.n), q_all)
    ...
    def get_reward_terms(self, q_all, u_all):
        def single_reward_terms(k, q, u_all):
            ...
            def single_obs_penalty_logbar(p, obstacle, margin=0.5, epsilon=1e-3):
                xc, yc, w, h = obstacle
                x_min = xc - w / 2
                x_max = xc + w / 2
                y_min = yc - h / 2
                ...
                safe_d = self.Ra + margin
                penalty = -jnp.log(jnp.clip((safe_d - dist) / margin, a_min=epsilon))
                return jnp.where(dist < safe_d, penalty, 0.0)

            if self.obstacles_enabled:
                r_obs_vals = jax.vmap(lambda obs: single_obs_penalty_logbar(p, obs))(self.static_obstacles)
