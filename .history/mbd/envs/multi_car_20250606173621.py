...
class Args:
    ...
    betaT: float = 4e-2         # final noise  # PER ECD DEVE ESSERE A 1 SE NO NON VAA
    # PER D4orm e ECD
    # beta0: float = 1e-4         # initial noise
    ...
    def __init__(self, n, radius=2.0, robot_radius=0.1,formation_shift = False,obstacles_enabled=False,ECD=False):
        ...
        if obstacles_enabled:
            ...
            # ])
                ...
                self.static_obstacles = jnp.array([
                    [0.0,  1.0, 3.0, 0.1],  # Ostacolo orizzontale in alto
                    [0.0, -1.0, 3.0, 0.1],  # Ostacolo orizzontale in basso
                    [-1.0, 0.0, 0.1, 3.0],  # Ostacolo verticale a sinistra
                    [ 1.0, 0.0, 0.1, 3.0],  # Ostacolo verticale a destra
                ])
    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            if self.obstacles_enabled:
                ...
                r_obstacles = 0.0
            def movement_penalty(q_seq):
                # q_seq shape: (T, n, state_dim)
                pos_seq = q_seq[:, :, :2]  # (T, n, 2)
                delta_pos = jnp.linalg.norm(pos_seq[1:] - pos_seq[:-1], axis=-1)  # (T-1, n)
                # Penalizza se delta_pos è piccola (poco movimento)
                penalty = jnp.mean(1.0 / (delta_pos + 1e-4))
                return -0.1 * penalty  # peso negativo per penalità
            # Supponiamo di avere q_all_seq passato come argomento
            r_movement = movement_penalty(q_all_seq)
            ...
            return r_goal +  r_safe + r_form + 0.1*r_control+1* r_obstacles+r_movement
