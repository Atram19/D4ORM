...
class MultiCar2d:
    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            pT = self.xg[k][:2]
            p0 = self.x0[k][:2] 
            dist_to_goal = jnp.linalg.norm(p - pT)  # distance to goal
            dist0 = jnp.linalg.norm(p0 - pT)  # initial distance to goal
            # r_goal = 1.0 - jnp.linalg.norm(p - pT) / jnp.linalg.norm(p0 - pT)
            r_goal = jnp.exp(-5.0 * (jnp.linalg.norm(p - pT) / (jnp.linalg.norm(p0 - pT) + 1e-6))**2)
            ...
            return 2*r_goal +  3*r_safe + r_form + 0.1*r_control+5* r_obstacles
    ...
    def get_reward_terms(self, q_all, u_all):
        def single_reward_terms(k, q, u_all):
            ...
            r_total = 2*r_goal + 0.2 * r_safe + r_form + 0.1 * r_control +0.5* r_obstacles
