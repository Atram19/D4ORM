...
def log_barrier_pairwise(p, others_pos, Ra, margin=0.05, epsilon=1e-3, scale=1.0):
    """
    Penalità log-barrier tra il robot in posizione p e gli altri in others_pos.

    Restituisce un valore più alto quando altri robot sono vicini (< 2*Ra + margin),
    zero o molto piccolo quando sono lontani.

    Args:
        p: posizione del robot corrente, shape (2,)
        others_pos: posizioni degli altri robot, shape (n-1, 2)
        Ra: raggio del robot
        margin: soglia aggiuntiva per la sicurezza
        epsilon: clipping minimo per stabilità numerica
        scale: fattore moltiplicativo della penalità

    Returns:
        Penalità media (log-barrier) rispetto agli altri robot.
    """
    dists = jnp.linalg.norm(p - others_pos, axis=1)
    dist_safe = jnp.clip(dists - 2 * Ra, a_min=epsilon)

    # Applica penalità solo se i robot sono troppo vicini
    penalty = jnp.where(dists < (2 * Ra + margin), -jnp.log(dist_safe), 0.0)

    return scale * jnp.mean(penalty)
...
class MultiCar2d:
    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            r_safe = -log_barrier_pairwise(p, q_all[:, :2], self.Ra)
    ...
    def get_reward_terms(self, q_all, u_all):
        def single_reward_terms(k, q, u_all):
            ...
            # dists = jnp.linalg.norm(p - q_all[:, :2], axis=1)
            # r_safe = -1.0 * jnp.any((dists <= 2 * self.Ra + 1e-2) & (jnp.arange(self.n) != k))
            r_safe = -log_barrier_pairwise_centered(p, q_all[:, :2], k, self.Ra)
