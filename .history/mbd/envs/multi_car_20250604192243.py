    ...
    def get_rewards(self, q_all, u_all):
        ...
        def single_reward(k, q,u_all):
            ...
            def single_obs_penalty(p, obstacle):
                ...
                margin = 0.1
                normalized = jnp.clip((self.Ra + margin - dist) / margin, a_min=1e-3)
                penalty = jnp.where(dist < self.Ra + margin, -jnp.log(normalized) + 1.0 / normalized, 0.0)
            ...
            if self.obstacles_enabled:
                ...
                r_obstacles = jnp.max(r_obs_vals)  # penalità massima
            else:
                r_obstacles = 0.0
