...
def run_diffusion_once(args: Args,env, rollout_us, reset_env_jit,step_env_jit):
    ...
    for t in range(U_0.shape[0]):
        ...
        rew_buffer.append(r_step)
        r_terms_from_step.append(r_terms)
        state = step_env_jit(state, u)
        x_all.append(state.pipeline_state)

    rew_buffer = jnp.stack(rew_buffer, axis=0)
    r_terms_from_step = jnp.stack(r_terms_from_step, axis=1)
    r_terms_from_step = jnp.transpose(r_terms_from_step, (1, 0, 2))
