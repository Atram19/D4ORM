...
def run_diffusion_once(args: Args,env, rollout_us, reset_env_jit):
    ...
    print("Shape dei reward:", rews.shape)  # deve stampare (T, n)
    print("Media per robot:", rews.mean(axis=0))
    ...
    for t in range(U_0.shape[0]):
        _, r_terms = env.get_rewards(pipeline_states[t+1], U_0[t])   # shape (n, 6)
        r_terms_all.append(r_terms)
        print("Shape:", r_terms_all.shape)



    r_terms_all = jnp.stack(r_terms_all, axis=0)  # shape (n, T, 6)
    print("Robot 0 r_total:", r_terms_all[0, :, -1])
    print("Media robot 0:", r_terms_all[0, :, -1].mean())
