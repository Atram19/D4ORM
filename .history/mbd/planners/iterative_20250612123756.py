...
def main():
    ...
    _, traj_init = rollout_us(state_init, U_init)
    traj = jnp.concatenate([state_init.pipeline_state[None], traj], axis=0)
    traj_init = jnp.concatenate([state_init.pipeline_state[None], traj_init], axis=0)
    traj = jnp.transpose(traj, (1, 0, 2))
    traj_init = jnp.transpose(traj_init, (1, 0, 2))
    ...
    if not args.not_render:
        ...
        for i in range(traj_init.shape[0]):
            ax.plot(traj_init[i, :, 0], traj_init[i, :, 1], '--', alpha=0.5, label=f'Robot {i} (init)', color='gray')
