...
def run_diffusion_local(args: Args, U_init: jnp.ndarray,env, rollout_us, reset_env_jit):
    ...
    if args.ECD:
        ...
        for i in range(final_ecd_iters):
            ...
            # Definito una sola volta, fuori dai loop
                ...
                for t_start in range(0, H - L + 1, L // 2):
                    ...
                    def reverse_once_local(U_w, rng_w):
                        ...
                        for j in reversed(range(1, L)):
                            Yi = Ybar_i * jnp.sqrt(alphas_bar_local[j])
                            rng_w, rng_eps = jax.random.split(rng_w)
                            eps_u = jax.random.normal(rng_eps, (args.Nsample, L, n, Nu))
                            Y0s = eps_u * sigmas_local[j] + Ybar_i
                            Y0s = jnp.clip(Y0s, -1.0, 1.0)
                            ...
                            weights = jax.nn.softmax(logp0)
                            Ybar = jnp.einsum("s,slij->lij", weights, Y0s)

                            score = 1.0 / (1.0 - alphas_bar_local[j]) * (-Yi + jnp.sqrt(alphas_bar_local[j]) * Ybar)
                            Yim1 = 1.0 / jnp.sqrt(alphas_local[j]) * (Yi + (1.0 - alphas_bar_local[j]) * score)
                            Ybar_i = Yim1 / jnp.sqrt(alphas_bar_local[j - 1])
                        return Ybar_i
