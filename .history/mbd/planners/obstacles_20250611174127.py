...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    def ecd_refinement_local(U_start, lambda_start, args, env, rollout_us, reset_env_jit):
        ...
        U_curr = U_start.copy()
        lambda_curr = lambda_start.copy()

        # Prepara le funzioni residual e lagrangiana
        state_for_goal = reset_env_jit(jax.random.PRNGKey(args.seed + 2048))
        residual_fn = make_residual_fn(state_for_goal, env, Nsample)
        lagrangian_fn = make_lagrangian_fn(state_for_goal, env, Nsample)

        for t_start in range(0, H - L + 1, L // 2):
            t_end = t_start + L
            U_window = U_curr[t_start:t_end]

            for i in range(N_inner):
                sigma_k = args.initial_sigma * jnp.exp(-args.noise_decay * i)
                sigma_k = jnp.where(i >= N_inner - 2, 0.0, sigma_k)

                rng_key = jax.random.PRNGKey(args.seed + 1024 + i + t_start)
                eps_u = jax.random.normal(rng_key, (Nsample, L, n, Nu))
                noise = eps_u * sigma_k
                Y0s_k = U_window + noise
                Y0s_k = jnp.clip(Y0s_k, -1.0, 1.0)

                # Ricrea il controllo completo
                U_fulls = jnp.repeat(U_curr[None, ...], Nsample, axis=0)
                U_fulls = U_fulls.at[:, t_start:t_end, :, :].set(Y0s_k)

                # Rollouts
                state_k = reset_env_jit(rng_key)
                rewss_k, pipeline_states_k = jax.vmap(rollout_us, in_axes=(None, 0))(state_k, U_fulls)
                pipeline_states_window = pipeline_states_k[:, t_start:t_end]

                # Lagrangiana
                L_cost, L_constraint, L_vals_k, *_ = lagrangian_fn(
                    Y0s_k, Y0s_k, pipeline_states_k, pipeline_states_window, lambda_curr, mu)
                L_vals_std = L_vals_k.std() + 1e-6
                L_vals_mean = L_vals_k.mean()
                L_vals_scaled = (L_vals_k - L_vals_mean) / L_vals_std

                # Gradiente
                alpha_r = 1.0   # peso reward
                alpha_l = 1e-3  # peso lagrangiana (scalata)

                stim = -1*(alpha_r * rews_k.mean(axis=(1, 2)) - alpha_l * L_vals_k)
                ...
                U_window = U_window - alpha * grad

                # Aggiornamento lambda (vincolo globale)
                h_goal = residual_fn(pipeline_states_k)
                h_mean = jnp.mean(h_goal, axis=0).reshape(-1)
                lambda_curr = lambda_curr + alpha * mu * h_mean

            # Inserisci la finestra ottimizzata in U_curr
            U_curr = U_curr.at[t_start:t_end].set(U_window)
    ...
    #U_refined, lambda_goal = ecd_refinement_local(U, lambda_new, args, env, rollout_us, reset_env_jit)
