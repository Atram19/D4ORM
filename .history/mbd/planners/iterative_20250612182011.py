...
def main():
    ...
    if not args.not_render:
        ...
        for t in range(U_opt.shape[0]):
            ...
            r_terms_all_glocal = jnp.concatenate([r_terms_all_global, r_terms[None]], axis=0)
       ...
       # Save final trajectory plot
        ...
        plot_and_save_reward_terms(r_terms_all_local, path=path, prefix=f"reward_local/reward_plot_{ecd_tag}_{formation_tag}",component_names=component_names)
        plot_and_save_reward_terms(r_terms_all_global, path=path, prefix=f"reward/globalreward_plot_{ecd_tag}_{formation_tag}",component_names=component_names)
