...
def run_diffusion_once(args: Args,env, rollout_us, reset_env_jit,step_env_jit):
    ...
    for t in range(U_init.shape[0]):
        ...
        # Reward calcolato come fa step()
        r_step, r_terms = env.get_rewards(q, u)
        rew_buffer.append(r_step)                # r_step: shape (n,)
        r_terms_all.append(r_terms)              # r_terms: shape (n, 6)

        state = step_env_jit(state, u)
        x_all.append(state.pipeline_state)

    # Stack finale
    rew_buffer = jnp.stack(rew_buffer, axis=0)           # (T, n)
    r_terms_all = jnp.stack(r_terms_all, axis=1)         # (n, T, 6)
    x_all = jnp.stack(x_all, axis=0)                     # (T+1, n, 3)
    x_all = jnp.transpose(x_all, (1, 0, 2))              # (n, T+1, 3)
    r_terms_all = jnp.transpose(r_terms_all, (1, 0, 2))  # (n, T, 6)

    # Confronti numerici
    print("Reward da rollout_us:", rewss_eval.mean(axis=0))
    print("Reward da state.reward:", rew_buffer.mean(axis=0))
    print("r_total da r_terms_all:", r_terms_all[:, :, -1].mean(axis=1))
    return U_0
