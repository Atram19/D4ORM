        ...
        for i in range(final_ecd_iters):
            ...
            for k in range(K):
                for t_start in range(0, H - L + 1, L // 2):  # sliding overlapping windows
                    ...
                    U_window = U[t_start:t_end]
                    sigma_local = sigmas_local[k] # senza ostacoli
                    ...
                    def reverse_once_local(U_w, rng_w):
                        Ui = U_w * jnp.sqrt(alphas_bar_local[i])
                        ...
                        # Final evaluation
                        score = 1.0 / (1.0 - alphas_bar_local[i]) * (-Ui + jnp.sqrt(alphas_bar_local[i]) * U_opt)
                        Yim1 = 1.0 / jnp.sqrt(alphas_local[i]) * (Ui + (1.0 - alphas_bar_local[i]) * score)
                        Ybar_i = Yim1 / jnp.sqrt(alphas_bar_local[i - 1])
