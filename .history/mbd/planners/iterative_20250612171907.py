...
def run_diffusion_once(args: Args,env, rollout_us, reset_env_jit):
    ...
    def reverse(YN, rng):
        ...
        for i in reversed(range(1,100)):
            carry = (i, rng, Yi)
            (i, rng, Yi),Yi_next,Y0s = reverse_once(carry)
            Yi_list.append(Yi)  # salva il controllo Yi a ogni step
            Y0s = Y0s.reshape(args.Nsample, args.Hsample, args.n_robots, env.action_size)
            Y0s_list.append(np.array(Y0s))  

        return Yi,Yi_list  # U^(0)

    rng_exp, rng = jax.random.split(rng)
    U_0,Yi_list = reverse(YN, rng_exp)
    ...
    Y0s_list_np = np.array(Y0s_list, dtype=object)  # Serve dtype=object per lista di array
    Yi_list_np = np.array(Yi_list, dtype=object)
    print(f"global robots average rewards: {reward_array_str}")

    np.savez("results/multicar_iterative/global_Yi_list.npz", Y0s_list=Y0s_list_np, Yi_list=Yi_list_np)
...
def run_diffusion_local(args: Args, U_init: jnp.ndarray,env, rollout_us, reset_env_jit):
    ...
    """
    rng = jax.random.PRNGKey(seed=args.seed + 1024)  
    rewards_per_iter = []
    ...
    if args.ECD:
        state_init_for_goal = reset_env_jit(jax.random.PRNGKey(args.seed + 1024))
...
def main():
    ...
    if not args.not_render:
        ...
        r_terms_all = jnp.array([jnp.zeros((args.n_robots, 6))])  # shape (1, n, 6)
        state = state_init
        for t in range(U_opt.shape[0]):
            ...
            r_terms_all = jnp.concatenate([r_terms_all, r_terms[None]], axis=0)
        xs = jnp.transpose(xs, (1, 0, 2))
        
        r_terms_all = jnp.transpose(r_terms_all, (1, 0, 2))  # (n, T+1, 6)
       ...
       # Save final trajectory plot
        ...
        for i in range(x_init.shape[0]):
            ax.plot(x_init[i, :, 0], x_init[i, :, 1], '--', alpha=0.5, color=cmap(i))
        ...
        plot_and_save_reward_terms(r_terms_all, path=path, prefix=f"reward_plot_{ecd_tag}_{formation_tag}",component_names=component_names)
