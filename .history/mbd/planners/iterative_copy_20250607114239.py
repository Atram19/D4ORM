...
def run_diffusion_once(args: Args,env, rollout_us, reset_env_jit):
    ...
    YN = jax.random.normal(rng, (args.Hsample, n, Nu)) * args.initial_sigma
...
def run_diffusion_local(args: Args, U_init: jnp.ndarray,env, rollout_us, reset_env_jit):
    ...
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env):
        ...
        N_inner = 5  # number of ECD iterations
        ...
        for i in range(N_inner):
            ...
            Y0s = eps_u * sigma_k + U_curr
            noise = eps_u*sigma_k
            noise_norm = jnp.linalg.norm(noise.reshape(Nsample, -1), axis=1).mean()
            Y0s = jnp.clip(Y0s, -1.0, 1.0)
            # print("DEBUG Y0s: min", Y0s.min(), "max", Y0s.max())
            # print("Y0s NaN?", jnp.isnan(Y0s).any())
            ...
            L_cost,L_constraint,L_tot,control_cost,barrier_cost,goal_cost,h_flat,obstacle_cost_local = lagrangian_fn(Y0s_window,Y0s, pipeline_states, pipeline_states_window, lambda_goal, args.mu)
            ...
            grad = grad / (Nsample * sigma_local**2 + 1e-8)
        ...
        #     grad = grad / (Nsample * sigma_local**2 + 1e-6)
            ...
            # print("U_next NaN:", jnp.isnan(U_next).any(), "min:", U_next.min(), "max:", U_next.max())

            h_goal = residual_fn(pipeline_states)  
            h_mean = jnp.mean(h_goal, axis=0).reshape(-1)
            
            lambda_goal = lambda_curr + args.alpha * args.mu * h_mean
            U_curr = U_next
            lambda_curr = lambda_goal
    ...
    for i in range(8):  # numero iterazioni locali
        ...
        for t_start in range(0, H - L + 1, L // 2):
            ...
            U_opt_local, lambda_goal,r_vals_window, goal_cost, barrier_cost, control_cost, h_flat = reverse_once_local_hybrid(U_window, rng_step,U, t_start, residual_fn,lagrangian_fn,lambda_goal,args,env)
...
def main():
    ...
    if not args.not_render:
        ...
        collision_tag = "collision" if args.obstacles_enabled else ""
        ...
        np.savez(f"results/multicar_iterative/optimized_data_{ecd_tag}_{formation_tag}_{collision_tag}.npz", U_opt=np.array(U_opt), traj=np.array(traj), goals=np.array(env.xg), rewards=np.array(rewards_per_iter))
        ...
        # === Disegna ostacoli statici ===
        if args.obstacles_enabled:
            if hasattr(env, "static_obstacles"):
