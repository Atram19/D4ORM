...
def run_diffusion_once(args: Args,env, rollout_us, reset_env_jit,step_env_jit):
    ...
    # Recupera i reward calcolati da step()
    rew_buffer = []
    state = state_init_eval
    for t in range(U_0.shape[0]):
        state = step_env_jit(state, U_0[t])
        rew_buffer.append(state.reward)

    rew_buffer = jnp.stack(rew_buffer, axis=0)  # shape (T, n)

    # === Confronti diretti ===
    print("→ reward_per_robot da rollout_us (media temporale):")
    print(rewss_eval.mean(axis=0))

    print("→ media r_total da r_terms:")
    print(r_total_means)

    print("→ media reward da state.reward (step_env_jit):")
    print(rew_buffer.mean(axis=0))

    print("→ diff tra rewss_eval e r_total:", rewss_eval.mean(axis=0) - r_total_means)
    print("→ diff tra rewss_eval e rew_buffer:", rewss_eval.mean(axis=0) - rew_buffer.mean(axis=0))
    print("→ diff tra r_total e rew_buffer:", r_total_means - rew_buffer.mean(axis=0))
