...
def run_diffusion_once(args: Args,env, rollout_us, reset_env_jit,step_env_jit):
    """
    ...
    rewss_eval, _ = rollout_us(state_init, U_0)
    r_terms_all = []
    
    for t in range(U_0.shape[0]):
        _, r_terms = env.get_rewards(state_init.pipeline_state, U_0[t])  # (n, 6)
        r_terms_all.append(r_terms)
        state_init = step_env_jit(state_init, U_0[t])
        
    r_terms_all = jnp.stack(r_terms_all, axis=1)  # shape (n, T, 6)
    r_total_means = r_terms_all[:, :, -1].mean(axis=1)  # media temporale di r_total

    
    print("r_total (da r_terms):", r_total_means)
    ...
    state = state_init
    for t in range(U_0.shape[0]):
        _, r_terms = env.get_rewards(state.pipeline_state, U_0[t])
        r_terms_direct.append(r_terms)
        state = step_env_jit(state, U_0[t])
    r_terms_direct = jnp.stack(r_terms_direct, axis=1)  # (n, T, 6)
    r_total_means_direct = r_terms_direct[:, :, -1].mean(axis=1)

    rew_buffer = []
    r_terms_from_step = []
    x_all = []

    state = reset_env_jit(jax.random.PRNGKey(args.seed + 1024))
    x_all.append(state.pipeline_state)

    for t in range(U_0.shape[0]):
        q = state.pipeline_state
        u = U_0[t]

        # ✅ Calcola reward (come fa step)
        r_step, r_terms = env.get_rewards(q, u)
        rew_buffer.append(r_step)          # r_step: shape (n,)
        r_terms_from_step.append(r_terms)  # r_terms: shape (n, 6)

        # ✅ Avanza lo stato
        state = step_env_jit(state, u)
        x_all.append(state.pipeline_state)

    # Stack e reshape finali
    rew_buffer = jnp.stack(rew_buffer, axis=0)                     # shape (T, n)
    r_terms_from_step = jnp.stack(r_terms_from_step, axis=0)       # shape (T, n, 6)
    r_terms_from_step = jnp.transpose(r_terms_from_step, (1, 0, 2))  # shape (n, T, 6)

    # Check shapes
    print("Shape r_terms_from_step:", r_terms_from_step.shape)

    # Confronti numerici coerenti
    print("Reward da rollout_us:           ", rewss_eval.mean(axis=0))
    print("Reward da state.reward:         ", rew_buffer.mean(axis=0))
    print("r_total da r_terms_from_step:   ", r_terms_from_step[:, :, -1].mean(axis=1))

    print("Reward da rollout_us:", rewss_eval.mean(axis=0))
    print("Reward da state.reward:", rew_buffer.mean(axis=0))
    print("r_total da r_terms_direct:", r_total_means_direct)
    print("r_total da r_terms_from_step:", r_terms_from_step[:, :, -1].mean(axis=1))
    rewss_eval_split, _ = rollout_us( state, U_0)
    print("Correct reward per robot from rollout_us_split:", rewss_eval_split.mean(axis=0))

    state1 = reset_env_jit(jax.random.PRNGKey(args.seed + 1024))
    state2 = reset_env_jit(jax.random.PRNGKey(args.seed + 1024))

    print("Stati iniziali uguali?", jnp.allclose(state1.pipeline_state, state2.pipeline_state))
    return U_0
...
def main():
    ...
    t1 = time.time()
    U_init = run_diffusion_once(args,env, rollout_us, reset_env_jit,step_env_jit)
    t2 = time.time()
    ...
    if not args.not_render:
        ...
        for t in range(U_init.shape[0]):
            q = state.pipeline_state               # q corrente
            u = U_init[t]                          # u corrente
            _, r_terms = env.get_rewards(q, u)     # reward corretto su (q, u)
            r_terms_all_local = jnp.concatenate([r_terms_all_local, r_terms[None]], axis=0)

            state = step_env_jit(state, u)         # aggiorna lo stato
