...
def run_diffusion_once(args: Args, env, rollout_us, reset_env_jit):
    ...
    Nu = env.action_size
    n = env.num_robots
    T = 99  # reverse steps  HO AUMENTATO A 99 INVECE DI 19 
...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env,sigmas_local,alphas_bar_local):
        ...
        for j in reversed(range(1,L)):
            ...
            U_w = jnp.sqrt(alphas_bar_local[j - 1]) * U_soft


        return U_w , r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,obstacles,L_cost,L_tot
    ...
    for i in range(20):  # iterazioni locali
        for t_start in range(0, H - L + 1, L // 2):
            ...
            U_opt_local, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,obstacles, L_cost,L_tot  = reverse_once_local_hybrid(
                U_window, rng_step, U, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env,sigmas_local,alphas_bar_local)
