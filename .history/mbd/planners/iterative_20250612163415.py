...
def run_diffusion_once(args: Args,env, rollout_us, reset_env_jit):
    ...
    def reverse_once(carry):
        ...
        return (i - 1, rng, Ybar_im1)
    Yi_list = []
    def reverse(YN, rng):
        Yi = YN
        for i in reversed(range(1,1000)):
            carry = (i, rng, Yi)
            (i, rng, Yi) = reverse_once(carry)
    ...
    Yi_array = np.stack([np.array(y) for y in Yi_list], axis=0)  # shape: (Nsteps, H, n, Nu)

    np.savez("results/multicar_iterative/global_Yi_list.npz", Yi_list=np.array(Yi_list))
