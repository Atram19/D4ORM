...
def run_diffusion_once(args: Args,env, rollout_us, reset_env_jit,step_env_jit):
    ...
    r_terms_all = []
    state = state_init_eval
    for t in range(U_0.shape[0]):
        _, r_terms = env.get_rewards(state.pipeline_state, U_0[t])  # (n, 6)
    ...
    print("r_total da r_terms_from_step:", r_terms_from_step[:, :, -1].mean(axis=1))
    rewss_eval_split, _ = rollout_us(step_env_jit, reset_env_jit(jax.random.PRNGKey(args.seed + 1024)), U_0)
