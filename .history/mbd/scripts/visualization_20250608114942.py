...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env,sigma_local):
        ...
        rews = rewss.mean(axis=(1,2))
        # Step 3: Lagrangiana
        L_cost,L_cost_global, L_constraint, L_tot, control_cost, barrier_cost, goal_cost, h_flat,obstacles = lagrangian_fn(
            Y0s_window, Y0s, pipeline_states, pipeline_states_window, lambda_goal, args.mu)

       
        score_vals = rews-L_cost_global
        ...
        #         noise = eps_u * sigma_k
        #         Y0s_k = U_curr + noise
        #         U_template_refined = U_full_template.at[t_start:t_start+L].set(U_soft)

        #         U_fulls_k = jnp.repeat(U_template_refined[None, ...], Nsample, axis=0)
        ...
        return U_soft, lambda_goal, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,obstacles,L_cost,L_tot,
    ...
    for i in range(20):  # iterazioni locali
        for t_start in range(0, H - L + 1, L // 2):
            ...
            U_opt_local, lambda_goal, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,obstacles, L_cost,L_tot  = reverse_once_local_hybrid(
