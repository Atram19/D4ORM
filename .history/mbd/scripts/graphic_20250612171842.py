...
Yi_list = data["Yi_list"]  # (100 - 1,) array di (H, n, Nu)# === Rollout dei campioni ===
state_init = reset_env_jit(jax.random.PRNGKey(args.seed + 1024))
trajectories_all = []  # ogni entry: (Nsample, T+1, n, 2)

for Y0s in Y0s_list:
    trajs, _ = jax.vmap(rollout_us, in_axes=(None, 0))(state_init, Y0s)
    trajs_xy = trajs[..., :2]  # shape (Nsample, T+1, n, 2)
    trajectories_all.append(np.array(trajs_xy))  # append (Nsample, T+1, n, 2)

state_init = reset_env_jit(jax.random.PRNGKey(args.seed + 1024))
trajectories_denoised = []  # ogni entry: (T+1, n, 2)

for Yi in Yi_list:
    traj, _ = rollout_us(state_init, Yi)
    traj_xy = traj[..., :2]  # shape: (T+1, n, 2)
    trajectories_denoised.append(np.array(traj_xy))

# === Crea video ===
output_path = "results/multicar_iterative/global_diffusion_samples_video.mp4"
...
def update(frame):
    ...
    for i in range(n):
        for s in range(min(100, samples.shape[0])):  # limitiamo a 100 sample per leggibilità
            traj = samples[s]  # shape: (T+1, n, 2)
            x = traj[:, i, 0]
            y = traj[:, i, 1]
            ax.plot(x, y, alpha=0.1, color=cmap(i))
    traj_opt = trajectories_denoised[frame]  # (T+1, n, 2)
    for i in range(n):
        x_opt = traj_opt[:, i, 0]
        y_opt = traj_opt[:, i, 1]
        ax.plot(x_opt, y_opt, color=cmap(i), linewidth=2.0, label=f"robot {i}" if frame == 0 else None)
