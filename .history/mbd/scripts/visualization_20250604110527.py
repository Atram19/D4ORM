...
def run_diffusion_once(args: Args, env, rollout_us, reset_env_jit):
    ...
    def reverse_once(carry, _):
        i, rng, Ybar_i, traj_buf, ybar_buf = carry
        ...
        eps_u = eps_u.at[..., 0].set(eps_u[..., 0] * 1.5)  # Amplifica la varianza su omega
        ...
        return (i - 1, rng, Ybar_im1, traj_buf, ybar_buf), None

    def reverse(YN, rng):
        traj_buf, ybar_buf = init_buffers()
        carry = (T, rng, YN, traj_buf, ybar_buf)
        (i_final, rng_final, U_0, traj_buf, ybar_buf), _ = jax.lax.scan(reverse_once, carry, None, length=T)
        print("ω_mean:", traj_buf[:, :, :, 0].mean(), "ω_std:", traj_buf[:, :, :, 0].std())

        return U_0, traj_buf, ybar_buf

    rng_exp, rng = jax.random.split(rng)
    U_0, sample_trajectories_xy, Ybar_list = reverse(YN, rng_exp)
...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env):
        ...
        return U_next, lambda_goal, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,sample_trajs_xy,U_xy,gradients_buffer
    ...
    for i in range(8):  # iterazioni locali
        ...
        for t_start in range(0, H - L + 1, L // 2):
            ...
            U_opt_local, lambda_goal, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat, sample_trajs_xy, U_xy,gradients_buffer  = reverse_once_local_hybrid(
