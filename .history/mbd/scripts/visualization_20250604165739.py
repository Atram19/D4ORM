...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env):
        ...
        grad = jnp.einsum("s,slij->lij", (r_vals_window - r_vals_window.mean()), noise)
        grad = grad / (Nsample * sigma_local**2 + 1e-6)
        ...
        return U_next, lambda_goal, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,obstacles,sample_trajs_xy,U_xy,gradients_buffer
    ...
    for i in range(8):  # iterazioni locali
        ...
        for t_start in range(0, H - L + 1, L // 2):
            ...
            U_opt_local, lambda_goal, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,obstacles, sample_trajs_xy, U_xy,gradients_buffer  = reverse_once_local_hybrid(
