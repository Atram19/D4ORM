...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    L_cost_all = []
    L_tot_all = []
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start,
                              residual_fn, lagrangian_fn, lambda_goal, args, env, sigma_local):

            Nsample = args.Nsample
            L, n, Nu = U_window.shape

            # === D4ORM Monte Carlo Sampling ===
            eps_u = jax.random.normal(rng_w, (Nsample, L, n, Nu))
            noise = eps_u * sigma_local
            Y0s = jnp.clip(U_window + noise, -1.0, 1.0)

            # Inserisci nella traiettoria completa
            ...
            # Lagrangiana
            L_cost, L_constraint, L_tot, control_cost, barrier_cost, goal_cost, h_flat, _ = \
                lagrangian_fn(Y0s, Y0s, pipeline_states, pipeline_states_window, lambda_goal, args.mu)
            ...
            # Media pesata (denoising D4ORM)
            U_soft = jnp.einsum("s,slij->lij", weights, Y0s)

            # === ECD Refinement (gradient descent) ===
            ...
            U_final, lambda_goal = ecd_refinement(U_soft, lambda_goal)

            return U_final, lambda_goal
