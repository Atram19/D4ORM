...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    L_cost_all = []
    L_tot_all = []
    def ecd_refinement(U_start, lambda_start, t_start, U_template, args, env,
            residual_fn, lagrangian_fn, reset_env_jit, rollout_us):
            """
            Refinement via ECD gradient descent on window [t_start:t_start+L]
            """
            Nsample = args.Nsample
            N_inner = 30
            L, n, Nu = U_start.shape
            ...
            for i in range(N_inner):
                ...
                rng_key = jax.random.PRNGKey(args.seed + 1234 + i)
                eps_u = jax.random.normal(rng_key, (Nsample, L, n, Nu))
                noise = eps_u * sigma_k
                Y0s = jnp.clip(U_curr + noise, -1.0, 1.0)

                # Inserisci nella traiettoria globale
                U_fulls = jnp.repeat(U_template[None, ...], Nsample, axis=0)
                U_fulls = U_fulls.at[:, t_start:t_start + L, :, :].set(Y0s)

                # Rollout
                state_init = reset_env_jit(rng_key)
                ...
                # Lagrangiana
                L_cost, L_constraint, L_vals, control_cost, barrier_cost, goal_cost, h_flat, _ = \
                    lagrangian_fn(Y0s, Y0s, pipeline_states, pipeline_states_window, lambda_curr, mu_k)

                # Gradiente REINFORCE
                grad = jnp.einsum("s,slij->lij", L_vals - L_vals.mean(), eps_u)
                grad = grad / (Nsample * sigma_k + 1e-8)

                U_next = U_curr - args.alpha * grad

                # Aggiorna i moltiplicatori di Lagrange
    ...
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env,sigma_k):
        ...
        # U_next = U_soft
        U_next, lambda_goal = ecd_refinement(U_soft, lambda_goal, t_start, U_full_template,
                args, env, residual_fn, lagrangian_fn, reset_env_jit, rollout_us)
