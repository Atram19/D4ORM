...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    def ecd_refinement(U_start, lambda_start, rews):
        ...
        U_soft_single = U_soft  # shape (L, n, Nu)
        U_full_single = U_full_template.at[t_start:t_start+L].set(U_soft_single)
        state_soft = env.reset(jax.random.PRNGKey(args.seed + 5555))
        rew_soft, pipeline_soft = rollout_us(state_soft, U_full_single)
        pipeline_soft_window = pipeline_soft[t_start:t_start+L]

        L_soft, _, L_tot_soft, *_ = lagrangian_fn(
            U_soft_single[None, ...], U_soft_single[None, ...], 
            pipeline_soft[None, ...], pipeline_soft_window[None, ...], 
            lambda_curr, args.mu
        )
...
baseline = rew_soft.mean() - L_tot_soft[0]
        ...
        for i in range(15):
            sigma_k = args.initial_sigma * jnp.exp(-i * args.noise_decay)
            sigma_k = sigma_k * jnp.sqrt(2 * args.alpha)  # match Euler-Maruyama
            ...
            noise = eps_u * sigma_k
            Y0s_k = jnp.clip(U_curr + noise, -1.0, 1.0)  # Sample attorno a U_curr
            ...
            U_next = U_curr - args.alpha * grad + sigma_k * eps_u.mean(axis=0)
            ...
            print(f"[ECD {i}] ||grad|| = {jnp.linalg.norm(grad):.3e}, ||U|| = {jnp.linalg.norm(U_curr):.3f}, ||λ|| = {jnp.linalg.norm(lambda_curr):.3e}")
