...
import functools
import os
from mbd.envs.multi_car import check_inter_robot_collisions, Arg
...
for Yi in Yi_list:
    _, traj = rollout_us(state_init, Yi)
    traj_xy = traj[..., :2]  # (T+1, n, 2)
    trajectories.append(traj_xy)

# Save animation
fig, ax = plt.subplots(figsize=(5, 5))
ax.set_aspect("equal")
lines = [ax.plot([], [], lw=1)[0] for _ in range(env.num_robots)]

def update(frame):
    ax.clear()
    traj = trajectories[frame]  # shape: (T+1, n, 2)
...
ani = animation.FuncAnimation(fig, update, frames=len(Yi_list), interval=100)
os.makedirs("results/multicar_iterative", exist_ok=True)
ani.save("results/multicar_iterative/global_diffusion_from_Yi.mp4", fps=5)
print("Video saved.")
