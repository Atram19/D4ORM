...
def run_diffusion_once(args: Args, env, rollout_us, reset_env_jit):
    ...
    def reverse_once(carry, _):
        ...
        # --- Aggiunta: gradiente di policy tipo REINFORCE ---
        # Differenza rispetto alla media (shape: Nsample, H, n, Nu)
        Y_diffs = Y0s - Ybar[None, :, :, :]  # broadcasting

        # Usa i pesi (dalla softmax) per media pesata (gradiente stimato)
        grad_reinforce = jnp.einsum("s,shij->hij", weights, Y_diffs)

        # Aggiorna Ybar con passo di gradiente
        Ybar = Ybar + 1 * grad_reinforce  # esempio: args.lr_reinforce = 0.01
        # Dopo l'aggiornamento di Ybar
        rng, rng_noise = jax.random.split(rng)
        directional_noise = 0.05 * jax.random.normal(rng_noise, Ybar.shape)  # 5% rumore
        Ybar = Ybar + directional_noise
