...
# Load Yi
data = np.load("results/multicar_iterative/global_Yi_list.npz", allow_pickle=True)
Yi_list = data["Yi_list"]  # shape: (Nsteps, H, n, Nu)
Y0s_list = data["Y0s_list"]  # shape: (Nsteps, Nsample, H, n, Nu)
# imposta o importa l'oggetto Args
env = MultiCar2d(n=Args.n_robots, formation_shift=Args.formation_shift, ECD=False, obstacles_enabled=Args.obstacles_enabled)
...
# === ROLLOUT FOR ALL SAMPLES ===
trajectories_all = []  # list of (Nsample, T+1, n, 2)
trajectories_mean = []  # list of (T+1, n, 2)

for Yi, Y0s in zip(Yi_list, Y0s_list):
    # rollout dei campioni
    rollout_Y0s, _ = jax.vmap(rollout_us, in_axes=(None, 0))(state_init, Y0s)
    rollout_Y0s_xy = rollout_Y0s[..., :2]  # shape: (Nsample, T+1, n, 2)
    trajectories_all.append(np.array(rollout_Y0s_xy))

    # rollout della traiettoria ottimizzata Yi
    _, traj_mean = rollout_us(state_init, Yi)
    traj_mean_xy = traj_mean[..., :2]
    trajectories_mean.append(np.array(traj_mean_xy))
output_path = "results/multicar_iterative/global_diffusion_from_samples.mp4"
...
def update(frame):
    ...
    traj_mean = trajectories_mean[frame]  # shape: (T+1, n, 2)
    ...
    for i in range(n):
        ...
        x_opt = traj_mean[:, i, 0]
        y_opt = traj_mean[:, i, 1]
        ax.plot(x_opt, y_opt, color=cmap(i), lw=2, label=f"Robot {i}")

    ax.legend()
    return []

ani = animation.FuncAnimation(fig, update, frames=len(Yi_list), interval=150)
