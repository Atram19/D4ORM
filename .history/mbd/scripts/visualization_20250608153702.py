...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    def ecd_refinement(U_start, lambda_start, rews):
        ...
        for i in range(30):
            ...
            print(f" stim.min = {stim.min():.4f}, stim.max = {stim.max():.4f}")
    ...
    for i in range(20):  # iterazioni locali
        for t_start in range(0, H - L + 1, L // 2):
            ...
            max_val = U.max()
            min_val = U.min()
            n_clipped_max = jnp.sum(U == 1.0)
            n_clipped_min = jnp.sum(U == -1.0)
            print(f"[] U ∈ [{min_val:.3f}, {max_val:.3f}], clipped +1: {n_clipped_max}, clipped -1: {n_clipped_min}")
