...
def init_guided_controls(state_init, goals, T):
    # state_init: shape (n, state_dim) (x, y, theta, ...)
    # goals: shape (n, state_dim) (x_goal, y_goal, ...)
    delta = goals[:, :2] - state_init[:, :2]  # solo posizione
    v_guess = delta / T  # velocità media per ogni robot

    v_controls = jnp.repeat(v_guess[:, None, :], T, axis=1)  # (n, T, 2)
    
    theta = jnp.arctan2(delta[:, 1], delta[:, 0])
    omega_controls = jnp.repeat((theta / T)[:, None], T, axis=1)  # (n, T)
    
    controls = jnp.concatenate([v_controls, omega_controls[..., None]], axis=-1)  # (n, T, 3)
    return controls
...
def run_diffusion_once(args: Args, env, rollout_us, reset_env_jit):
    ...
    betas = jnp.linspace(args.beta0, args.betaT, T + 1)
    alphas = 1.0 - betas
    ...
    guided_controls = init_guided_controls(state_init.pipeline_state, env.xg, args.Nsample)
    YN = jnp.repeat(guided_controls[None, :, :], args.Hsample, axis=0)  # (Hsample, n, Nu)
...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    L_cost_all = []
    L_tot_all = []
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env):
    ...
    noise= []
    for i in range(8):  # iterazioni locali
        ...
        for t_start in range(0, H - L + 1, L // 2):
            ...
            U_opt_local, lambda_goal, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,obstacles, L_cost,L_tot,sample_trajs_xy, U_xy,gradients_buffer  = reverse_once_local_hybrid(
                U_window, rng_step, U, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env)
