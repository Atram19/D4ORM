...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env, sigma_k):
        ...
        Y0s_window = Y0s  # serve per chiarezza più avanti
        ...
        # === STEP D4ORM: selezione U_soft basata su reward ===
        r_std = (r_vals_window - r_vals_window.mean()) / (r_vals_window.std() + 1e-6)
        logp0 = r_std / args.temp_sample
        weights = jax.nn.softmax(logp0)
        U_soft = jnp.einsum("s,slij->lij", weights, Y0s)

        # === STEP ECD: stimare gradiente sulla L_tot a partire da U_soft ===
        eps_u_ecd = jax.random.normal(rng_w, (Nsample, L, n, Nu))
        Y_ecd = eps_u_ecd * sigma_local + U_soft  # sample intorno a U_soft
        noise_ecd = eps_u_ecd * sigma_local

        # Inseriamo Y_ecd nella traiettoria completa
        U_fulls_ecd = jnp.repeat(U_full_template[None, ...], Nsample, axis=0)
        U_fulls_ecd = U_fulls_ecd.at[:, t_start:t_start+L, :, :].set(Y_ecd)
        rewss_ecd, pipeline_states_ecd = jax.vmap(rollout_us, in_axes=(None, 0))(state_init, U_fulls_ecd)
        pipeline_states_window_ecd = pipeline_states_ecd[:, t_start:t_start+L]

        # Lagrangiana completa per stimare gradiente
        _, _, L_tot_ecd, *_ = lagrangian_fn(
            Y_ecd, Y_ecd, pipeline_states_ecd, pipeline_states_window_ecd, lambda_goal, args.mu)

        L_tot_std = (L_tot_ecd - L_tot_ecd.mean()) / (L_tot_ecd.std() + 1e-6)
        grad = jnp.einsum("s,slij->lij", (L_tot_std - L_tot_std.mean()), noise_ecd)
