...
def run_diffusion_once(args: Args, env, rollout_us, reset_env_jit):
    ...
    Nu = env.action_size
    n = env.num_robots
    T = 49  # reverse steps  HO AUMENTATO A 59 INVECE DI 19 

    rng, rng_reset = jax.random.split(rng)
    state_init = reset_env_jit(rng_reset)

    # Diffusion noise schedule
    betas = jnp.linspace(args.beta0, args.betaT, 50)
    alphas = 1.0 - betas
    alphas_bar = jnp.cumprod(alphas)
    sigmas = jnp.sqrt(1 - alphas_bar)

    YN = jax.random.normal(rng, (args.Hsample, n, Nu)) * sigmas[-1]
    ...
    def reverse_once(carry, _):
        ...
        # Esegui rollout per ottenere traiettorie complete
        _, trajs = jax.vmap(rollout_us, in_axes=(None, 0))(state_init, Y0s)  # (S, H, n, 3)
        s0 = state_init.pipeline_state
        s0_batched = jnp.repeat(s0[None, ...], args.Nsample, axis=0)
        trajs_full = jnp.concatenate([s0_batched[:, None, :, :], trajs], axis=1)  # (S, H+1, n, 3)

        # Valuta potenziale esplicito
         def compute_total_potential(traj, env):
            """
            Potenziale esplicito: goal + collisioni + ostacoli
            traj: (H+1, n, 3)
            """
            H, n, _ = traj.shape
            goal_pos = env.xg[:, :2]
            ...
            def pot_timestep(q_t):
                pos = q_t[:, :2]
                # Goal
                dist_goal = jnp.linalg.norm(pos - goal_pos, axis=1)
                U_goal = jnp.sum(dist_goal ** 2)
                ...
                safe_dist = 2 * Ra + 0.05
                U_coll = jnp.sum(jnp.where(dists < safe_dist, -jnp.log(jnp.clip(safe_dist - dists, 1e-3)), 0.0))

                # Ostacoli fissi
                U_obs = 0.0
                for x_c, y_c, w, h in env.static_obstacles:
                    x_min, x_max = x_c - w/2, x_c + w/2
                    ...
                    margin = 0.1
                    U_obs += jnp.sum(jnp.where(dist < margin, -jnp.log(jnp.clip(margin - dist, 1e-3)), 0.0))

                return U_goal + U_coll + U_obs

            return jnp.sum(jax.vmap(pot_timestep)(traj))

        potentials = jax.vmap(lambda traj: compute_total_potential(traj, env))(trajs_full)  # (S,)
        weights = jax.nn.softmax(-potentials / args.temp_sample)
        Ybar = jnp.einsum("s,shij->hij", weights, Y0s)
...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env,sigma_k):
        ...
        sample_trajs_xy = pipeline_states_window[..., :2]               # (Nsample, L, n, 2)
        sample_buffer.append(np.array(sample_trajs_xy))
        ...
        L_tot_local = L_cost + L_constraint
        L_tot_local_std = -(L_tot_local - L_tot_local.mean()) / (L_tot.std() + 1e-6)

        L_std = (L_cost - L_cost.mean()) / (L_cost.std() + 1e-6)
        score_vals = r_std - alpha * L_tot_local_std

        logp0 = (score_vals - score_vals.mean()) / (score_vals.std() + 1e-6) / args.temp_sample
        weights = jax.nn.softmax(logp0)
        print("L_tot NaN:", jnp.isnan(L_tot).any(), "min:", L_tot.min(), "max:", L_tot.max())
        print("L_cost NaN:", jnp.isnan(L_cost).any(), "min:", L_cost.min(), "max:", L_cost.max())
        print("rewards NaN:", jnp.isnan(rewss).any(), "min:", rewss.min(), "max:", rewss.max())
        print("score_vals NaN:", jnp.isnan(score_vals).any())
        # Step 4: Denoising e update
        U_soft = jnp.einsum("s,slij->lij", weights, Y0s)
        #grad = jnp.einsum("s,slij->lij", (score_vals - score_vals.mean()), noise)
        L_tot_std = (L_tot- L_tot.mean()) / (L_tot.std() + 1e-6)
        grad = jnp.einsum("s,slij->lij", (L_tot_std - L_tot_std.mean()), noise)

        grad = grad / (Nsample * sigma_local**2 + 1e-6)
        print("grad NaN:", jnp.isnan(grad).any(), "min:", grad.min(), "max:", grad.max())

        U_next = U_soft- 0.01*grad 
        gradients_buffer.append(np.array(grad)) 
        ...
        # === SALVA SAMPLE Y0s E U_NEXT COME TRAIETTORIE ===
        state_vis = env.reset(jax.random.PRNGKey(args.seed + 1024))
        U_fulls_vis = jnp.repeat(U[None, ...], args.Nsample, axis=0)
        _, pipeline_states = jax.vmap(rollout_us, in_axes=(None, 0))(state_vis, U_fulls_vis)
        trajs_xy = pipeline_states[..., :2]  # (Nsample, H, n, 2)

        # rollout U ottimo
        _, pipeline_states_opt = rollout_us(state_vis, U)
        U_xy = pipeline_states_opt[:, :, :2].transpose(1, 0, 2)
        

        return U_next, lambda_goal, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,obstacles,L_cost,L_tot,sample_trajs_xy,U_xy,gradients_buffer
    ...
    for i in range(20):  # iterazioni locali
        ...
        sigma_k = jnp.where(i >= 20 - 2, 0.0, sigma_k)
        noise.append(sigma_k)
        for t_start in range(0, H - L + 1, L // 2):
            t_end = t_start + L
            U_window = U[t_start:t_end]
            rng, rng_step = jax.random.split(rng)

            U_opt_local, lambda_goal, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,obstacles, L_cost,L_tot,sample_trajs_xy, U_xy,gradients_buffer  = reverse_once_local_hybrid(
                U_window, rng_step, U, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env,sigma_k)
            ...
            J_obstacles_all.append(np.array(obstacles)) 
            L_cost_all.append(np.array(L_cost))
            L_tot_all.append(np.array(L_tot))
            trajectories_xy_all.append(np.array(sample_trajs_xy))
            trajectory_buffer.append(np.array(U_xy))
            # === Salva anche la traiettoria finale ottimizzata ===
            # state_final = env.reset(jax.random.PRNGKey(args.seed + 999))
            # _, pipeline_states_final = rollout_us(state_final, U)
            # final_traj_xy = pipeline_states_final[:, :, :2].transpose(1, 0, 2)  # (n, H, 2)
            # trajectory_buffer.append(np.array(final_traj_xy))
    ...
    np.savez("results/multicar_iterative/trend_samples_iter_7.npz",
             ...
             J_obstacles = np.stack(J_obstacles_all),
             L_cost = np.stack(L_cost_all),
             L_tot = np.stack(L_tot_all),
             H_norm=np.stack(H_norm_all),
             trajectories_xy=trajectories_xy_all,
             trajectory_buffer=trajectory_buffer,
             gradients_buffer=gradients_buffer, noise_norm=np.stack(noise)) 
...
def main():
    ...
    U_opt = run_diffusion_once(args,env, rollout_us, reset_env_jit)
