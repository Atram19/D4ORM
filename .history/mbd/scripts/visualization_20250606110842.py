...
def run_diffusion_once(args: Args, env, rollout_us, reset_env_jit):
    ...
    Nu = env.action_size
    n = env.num_robots
    T = 59  # reverse steps  HO AUMENTATO A 59 INVECE DI 19 
...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env):
        ...
        import matplotlib.pyplot as plt
        plt.plot(score_vals, label="score_vals (reward-centered)")
        plt.plot(L_tot_std, label="L_tot_std (normalized constraint)")
        plt.legend()
        plt.grid()
...
def main():
    ...
    env = MultiCar2d(n=args.n_robots, formation_shift=args.formation_shift, obstacles_enabled=args.obstacles_enabled)
