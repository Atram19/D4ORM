...
def run_diffusion_local(args: Args, U_init: jnp.ndarray, env, rollout_us, reset_env_jit):
    ...
    def reverse_once_local_hybrid(U_window, rng_w, U_full_template, t_start, residual_fn, lagrangian_fn, lambda_goal, args, env,sigma_local):
        ...
        def ecd_refinement(U_start, lambda_start,rews):
            ...
            for i in range(30):
                ...
                sigma_k = jnp.where(i >= 28, 0.0, sigma_k)
                mu_k = args.mu

                rng_key = jax.random.PRNGKey(args.seed + 9999 + i)
                eps_u = jax.random.normal(rng_key, (Nsample, L, n, Nu))
                noise = eps_u * sigma_k
                Y0s_k = jnp.clip(U_curr + noise, -1.0, 1.0)

                U_fulls_k = jnp.repeat(U_full_template[None, ...], Nsample, axis=0)
                U_fulls_k = U_fulls_k.at[:, t_start:t_start+L, :, :].set(Y0s_k)

                state_k = env.reset(rng_key)
                rewss_k, pipeline_states_k = jax.vmap(rollout_us, in_axes=(None, 0))(state_k, U_fulls_k)
                pipeline_states_window_k = pipeline_states_k[:, t_start:t_start+L]
                rews_k = rewss_k.mean(axis=(1,2))
                _, _, L_vals_k, *_ , h_flat_k, _ = \
                    lagrangian_fn(Y0s_k, Y0s_k, pipeline_states_k, pipeline_states_window_k, lambda_curr, mu_k)
                stim = rews_k-L_vals_k
        ...
        # Applica refinamento ECD
        U_next, lambda_goal = ecd_refinement(U_soft, lambda_goal,rews)
        ...
        return U_next, lambda_goal, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,obstacles,L_cost,L_tot,sample_trajs_xy,U_xy
    ...
    for i in range(20):  # iterazioni locali
        for t_start in range(0, H - L + 1, L // 2):
            ...
            U_opt_local, lambda_goal, r_vals_window, goal_cost, barrier_cost, control_cost, h_flat,obstacles, L_cost,L_tot,sample_trajs_xy, U_xy  = reverse_once_local_hybrid(
