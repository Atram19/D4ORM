...
# === 2. Trova tutti i file che iniziano con optimized_data_ e finiscono con .npz
candidates = [
...
# === 3. Ordina per data di modifica (più recente per ultimo)
candidates = sorted(
    candidates,
    key=lambda f: os.path.getmtime(os.path.join(path, f))
)

# === 4. Prendi il file più recente
filename = candidates[-1]
print(f"Caricato file più recente: {filename}")

# === 5. Determina il suffisso dinamicamente
if "ecd" in filename:
    ecd_tag = "ecd"
...
goals = data['goals']     # (n, 3)
rewards = data['rewards'] # (K, n)
